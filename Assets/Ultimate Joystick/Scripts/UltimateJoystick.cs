...
public class UltimateJoystick : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
	...
	void Start ()
	{
        ...
        if ( Application.isPlaying == false )
			return;
        if (GUIManager.Instance != null) GUIManager.Instance.SetJoyStick(gameObject);
	...
	Vector2 ConfigureImagePosition ( Vector2 textureSize, Vector2 customSpacing )
	{
        ...
        switch (anchor) {
            case Anchor.Left:
                ...
                break;
            case Anchor.Center:
                tempVector.x = Screen.width / 2 - textureSize.x / 2;
                break;
            case Anchor.Right:
	...
	void UpdateSizeAndPlacement ()
	{
		...
		else
		{
			...
			Vector2 tempVector = new Vector2(Screen.width , Screen.height);// new Vector2( textureSize, textureSize );
			
			// Apply the joystick size multiplied by the fixedTouchSize.
			baseTrans.sizeDelta = tempVector;
